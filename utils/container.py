...
from utils.jobscript import Jobscript
# from utils.convert_finished import Convert_finished
...
class TableContent(QTableWidget):
    ...
    def __init__(self, width=None, height=None, current_category=None, top_level_category=None,
                 ...
                 info_status=None, progress=None):
        """
        ...
        self.video_formats = [".mov", ".avi", ".mp4", ".R3D"]
    ...
    def read_fav_db():
        """
        load the database from fav_data.json.
        :return dict: database.
        """
        db = Database()
        return db.read_from_fav_json()
    ...
    def dropEvent(self, event):
        ...
        elif self.group == 'Reference':
            if os.environ.get('HOSTNAME') in config['LEADER']:
                if event.mimeData().urls:
                    dropped_files = []
                    event.setDropAction(Qt.CopyAction)
    ...
    def validate_sequence(file, other_formats, img_formats):
        ...
        if not os.path.isdir(file):
            if not os.path.splitext(file)[1] in other_formats and \
                ...
                p = re.compile('[0-9]+')
                if p.match(os.path.basename(file)) == None:
                    ...
                    file_seqs = pyseq.get_sequences(
                        "{}/{}{}*".format(os.path.dirname(file),
                                          p.findall(os.path.basename(file))[0],
                                          re.findall(r"\D+", os.path.basename(file))[0]
                                          )
                    )
    ...
    def load_thumbnail(self):
        ...
        fav_group = {}
        try:
            fav_db = TableContent.read_fav_db()[self.group][self.top_level_category][self.current_category]
            if fav_db == {}:
                pass
            else:
                for key, value in fav_db.items():
        ...
        file_list = []
        for item in TableContent.query_items_by_filters(
                self.current_category, self.top_level_category, self.group, self.d_type, self.search):
            if fav_group.get(item[3]) == None:
                fav_state = 'False'
            else:
                fav_state = str(fav_group[item[3]])

            if item[2] == self.current_category and (fav_state == str(self.favourite) or fav_state == "True"):
        ...
        try:
            ...
            for position, f_tuple in zip(cell_position, file_list):
                if f_tuple:
                    ...
                    count += 1
                    self.progress(count, len(file_list))
            self.cell_position_updated = True
        ...
        self.disable_cells()
        finish = time.time()
        print('Finished in {} second(s)'.format(str(round(finish-start, 2))))
    ...
    def thread_finished(self, data):
        ...
        data = (data[0].split("$$")[0], data[1], data[2])
        thumbnail = Thumbnail(self, item=data[1], source=data[0])
    ...
    def context_menu(self, pos):
        ...
        if sel_items:
            ...
            metadata = self.menu.addAction(QIcon("{}/info.png".format(config['icons'])), 'Metadata')

            if 'False' in fav_state:
                favourite = self.menu.addAction(
                    QIcon("{}/fav.png".format(config['icons'])), "Add to Favourite"
                )
            ...
            if "True" in fav_state:
                remove_favourite = self.menu.addAction(
                    QIcon("{}/remove_fav.png".format(config['icons'])), "Remove from Favourite"
                )
            ...
            if os.environ.get('HOSTNAME') in config['ADMIN']:
                if sel_item:
                    re_cache = self.menu.addAction(
                        QIcon("{}/recache.png".format(config['icons'])), "Recache"
                    )
                    re_cache.triggered.connect(self.re_cache)

                delete_cache = self.menu.addAction(
                    QIcon("{}/delete.png".format(config['icons'])), "Delete"
                )
                delete_cache.triggered.connect(self.delete_confirmation)

            self.menu.addSeparator()

            play_preview = self.menu.addAction(
                QIcon("{}/play.png".format(config['icons'])), "Play in RV"
            )

            explore = self.menu.addAction(
                QIcon("{}/explore.png".format(config['icons'])), "Show in Explorer"
            )

            pre_item = []
            for i in sel_items:
                if self.group == 'Reference':
                    clip = os.path.basename(i).split('.')[0] + '.mov'
                    preview_path = os.path.join(config['reference'],
                                                self.top_level_category,
                                                self.current_category,
                                                clip)
                    pre_item.append(preview_path)
                else:
                    if '$$' in i:
                        folder_name = os.path.dirname(i).split('/')[-1]
                        basename = os.path.splitext(os.path.basename(i))[0].split('%0')[0].rstrip('._') + '.mov'
                        clip = folder_name + '@' + basename
                    else:
                        clip = os.path.splitext(os.path.basename(i))[0].split('%0')[0].rstrip('._') + '.mov'
                    preview_path = os.path.join(config['proxy'],
                                                self.group,
                                                self.top_level_category,
                                                self.current_category,
                                                clip)
            ...
            play_preview.triggered.connect(lambda: TableContent.play_in_rv(pre_item))
    ...
    def re_cache(self):
        ...
        cell_position = \
            [(self.indexFromItem(item).row(), self.indexFromItem(item).column())
                         for item in self.selectedItems()]
    ...
    @staticmethod
    def play_in_rv(pre_item):
        """
        open selected files location in explorer/finder
        :param list pre_item: selected preview mov list
        :return: None
        """
        mov_path = ' '.join(pre_item)
        if sys.platform == "linux2":
            os.system("rv -play {} &".format(mov_path))
        ...
        for db_key, db_value in db.items():
            for src_file, fav_state in db_value.items():
                if src_file in sel_items:
                    Database().update_items_into_fav_json(
                        src_file,
                        self.current_category,
                        self.top_level_category,
                        self.group,
                    ...
                    if fav == False:
                        Database().remove_item_from_fav_json(
                            self.current_category,
                            self.top_level_category,
                            self.group,
                            db_key
...
class ConvertGif(QRunnable, QWidget, Jobscript):
    ...
    def __init__(self, input_file, gif_file, pos, frame_length, ffmpeg):
        ...
        self.pos = pos
        self.group = self.output.split('/')[-4]
        self.top_level_category = self.output.split('/')[-3]
        self.current_category = self.output.split('/')[-2]
        if 'Reference' in self.group:
            ref_forder = config['reference'] + '/' + self.top_level_category + '/' + self.current_category
            self.output_mov = ref_forder + '/' + os.path.basename(self.output).split('.')[0] + '.mov'
        else:
            self.output_mov = os.path.splitext(self.output)[0].split('%0')[0].rstrip('._') + '.mov'
    ...
    def run(self):
        ...
        if not os.path.isfile(self.output):
            if not '/m83_lib/2D_source_collect' in self.input and self.group == 'Comp_Source':
                self.messageBox('wrong path')
                return
            if self.output.split('.')[-1] in 'png':
                cmds = [self.ffmpeg,
                        "-i",
                        self.input,
                        "-vf",
                ...
                if out:
                    ...
                    fin_data = {'item': self.item.split('$$')[0], 'current_category': self.current_category,
                                'top_level_category': self.top_level_category,
                                'group': self.group, 'output': os.path.basename(self.output)}
                    pythonpath = "/core/Linux/APPZ/Python/2.7/bin/python"
                    convert_fin = "{0}/utils/convert_finished.py".format(Eleve_root)
                    subprocess.call('{0} {1} "{2}"'.format(pythonpath, convert_fin, fin_data), shell=True)
                if err:
                    print(err.decode())
                    self.conversion_signals.thread_finished.emit((self.item, self.output, self.pos))

            elif 'Reference' in self.group:
                if self.isHangul(os.path.basename(self.input)):
                    self.warning_messageBox()
                    return
                with open(self.output + '.tmp', 'w') as f:
                    f.write(self.item + ' >> ' + self.output)
                self.submit_to_farm()
                self.conversion_signals.thread_finished.emit((self.output_mov, self.output, self.pos))
    ...
    def messageBox(self, type):
        msgBox = QMessageBox()
        msgBox.setWindowTitle("Eleve Information")
        msgBox.setWindowIcon(QPixmap("info.png"))
        msgBox.setIcon(QMessageBox.Information)
        if type == 'exist file':
            msgBox.setText("The same file name exists.\n\n" \
                           "Please rename the file.")
        elif type == 'wrong path':
            msgBox.setText("Please check the path is correct.\n\n" \
                           "2D Source path must exist in '/m83_lib/2D_source_collect'")
        else:
            msgBox.setText("An error has been occurred.\n\n" \
                           "Please contact to TD.")
    ...
    def warning_messageBox(self):
        msgBox = QMessageBox()
        msgBox.setWindowTitle("Eleve Warning")
        msgBox.setWindowIcon(QPixmap("warning.png"))
        msgBox.setIcon(QMessageBox.Information)
        msgBox.setText("File names can only be used in English.\n\n" \
                       "Please change to English and try again.")
        msgBox.setInformativeText("")
        msgBox.exec_()
...
class RemoveCache(QRunnable):
    ...
    def __init__(self, cache):
        ...
        self.cache = cache
        self.group = self.cache.split('/')[-4]
        self.top_level_category = self.cache.split('/')[-3]
        self.current_category = self.cache.split('/')[-2]
        basename = os.path.splitext(os.path.basename(cache))[0].split('%0')[0].rstrip('._')+'.mov'
        self.cache_mov = os.path.join(os.path.dirname(cache), basename)
    ...
    def run(self):
        ...
        try:
            ...
            os.remove(self.cache_mov)
            if self.group == 'Reference':
                ref_forder = config['reference'] + '/' + self.top_level_category + '/' + self.current_category
                ref_mov = ref_forder + '/' + os.path.basename(self.cache).split('.')[0] + '.mov'
                os.remove(ref_mov)
