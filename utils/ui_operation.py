...
class UpdateUI:
    ...
    def save_settings(self, settings):
        ...
        self.settings.write_settings(
            settings.category_combobox.currentText(),
    ...
    def update_settings_ui(settings):
        ...
        settings.category_combobox.setCurrentText(data["category"])
        ...
        if is_group:
            ...
            if not self.ui.tree_category.currentItem().text(0) == "root":
                if self.ui.tree_category.currentItem().parent().text(0) == "root":
                    ...
                    if group == 'Reference':
                        self.remove_ref_directory(
                            "{}".format(self.ui.tree_category.currentItem().text(0))
                        )
    ...
    @staticmethod
    def make_ref_directory(directory):
        """
        create directory if not exists
        """
        settings_file = UpdateUI.load_settings()
        if not os.path.isdir("{}/{}".format(settings_file["reference"], directory)):
            os.makedirs("{}/{}".format(settings_file["reference"], directory))
    ...
    @staticmethod
    def rename_ref_directory(src, dest):
        """
        update directory name once renamed group/category
        """
        settings_file = UpdateUI.load_settings()
        if not os.path.isdir("{}/{}".format(settings_file["proxy"], dest)):
            os.rename(
                "{}/{}".format(settings_file["reference"], src),
                "{}/{}".format(settings_file["reference"], dest)
            )
    ...
    @staticmethod
    def remove_ref_directory(directory):
        """
        try to remove Reference directory from proxy
        """
        settings_file = UpdateUI.load_settings()
        if os.path.exists("{}/{}".format(settings_file["reference"], directory)):
            try:
                shutil.rmtree("{}/{}".format(settings_file["reference"], directory))
            except Exception:
                pass
    ...
    def add_category(self):
        ...
        if self.ui.combo_group.currentText():
            ...
            if current_item.text(0) == "root":
                ...
                if self.ui.combo_group.currentText() == 'Reference':
                    self.make_ref_directory("{}".format(item.text(0)))
                ...
                if self.ui.combo_group.currentText() == 'Reference':
                    self.make_ref_directory("{}/{}".format(
                        self.ui.tree_category.currentItem().text(0),
                        item.text(0)
                    ))
        else:
        ...
        if updated_data.parent():
            if updated_data.parent().text(0) == "root":
                if not updated_data.text(0).lower() in \
                    ...
                    if group_name == 'Reference':
                        self.rename_ref_directory(
                            "{}".format(current_data),
                            "{}".format(updated_data.text(0))
                        )
                else:
            ...
            # update sub-category
                ...
                if not updated_data.text(0).lower() in \
                    ...
                    if group_name == 'Reference':
                        self.rename_ref_directory(
                            "{}/{}".format(parent_name, current_data),
                            "{}/{}".format(parent_name, updated_data.text(0))
                        )
                else:
                    self.ui.tree_category.currentItem().setText(0, current_data)
            self.load_thumbnail_widget(
